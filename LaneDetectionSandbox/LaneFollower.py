...
class LaneFollower:
    ...
    def __init__(self):
        ...
        self.runNavigation = True

        self.croppedHeightRatio = (1.0/2.0)
        self.minSlope = 0.4
    ...
    #     leftLineImg = cv2.inRange(imgHSV, self.leftColorMin, self.leftColorMax)

    #     rightLineImg = cv2.inRange(imgHSV, self.rightColorMin, self.rightColorMax)
    ...
    def updateController(rightProcessed, leftProcessed):
        ...
        cropVertices = [(0, imgHeight),               # Corners of cropped image
                        (0, imgHeight * self.croppedHeightRatio),        # Gets bottom third
                        (imgWidth, imgHeight * self.croppedHeightRatio),
                        (imgWidth, imgHeight) ]
        ...
        for line in rightLines:
            for x1, y1, x2, y2 in line:
                slope = (y2-y1) / (x2 - x1)
                if ((math.fabs(slope) > self.minSlope) & (slope>=0)):
        ...
        centerPoint = (self.carCenter, imgHeight)
        cv2.line(finalImg, (centerPoint), (intPoint[0], intPoint[1]), (0,255,0), 5)

        # Calculate angle off of center
        midPoint = (self.carCenter, int(imgHeight/2))              # Center of screen. Used as a vertical reference for angle of departure
        cv2.line(finalImg, (centerPoint), (midPoint), (100, 200, 200), 4)
        angle = calculateAngle(centerPoint, intPoint, midPoint)

        angle = angle * 180/np.pi # is this angle * 180/pi instead of 180/pi???

        return [angle, finalImg]

    def runLaneDetection(self, frame):
