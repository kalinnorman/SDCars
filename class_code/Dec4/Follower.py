...
class Follower:
    def __init__(self):
        ...
        self.intersectionStops = cv2.imread('../MilestoneNov25/Maps/IntersectionStops.bmp') # RGB
        ...
        self.regions1and4 = cv2.imread('../MilestoneNov25/Maps/Region1to1_4to4.bmp') # Reads in the RGB image
        self.regions1and4 = cv2.cvtColor(self.regions1and4, cv2.COLOR_BGR2GRAY) # Convert to grayscale
        self.region1to4 = cv2.imread('../MilestoneNov25/Maps/Region1to4.bmp') # RGB
        self.region1to4 = cv2.cvtColor(self.region1to4, cv2.COLOR_BGR2GRAY) # Grayscale
        self.region1to3 = cv2.imread('../MilestoneNov25/Maps/Region1to3.bmp') # RGB
        self.region1to3 = cv2.cvtColor(self.region1to3, cv2.COLOR_BGR2GRAY) # Grayscale
        self.regions2and3 = cv2.imread('../MilestoneNov25/Maps/Region2to3_3to2.bmp') # RGB
        self.regions2and3 = cv2.cvtColor(self.regions2and3, cv2.COLOR_BGR2GRAY) # Grayscale
        self.region2to2 = cv2.imread('../MilestoneNov25/Maps/Region2to2.bmp') # RGB
        self.region2to2 = cv2.cvtColor(self.region2to2, cv2.COLOR_BGR2GRAY) # Grayscale
        self.region2to4 = cv2.imread('../MilestoneNov25/Maps/Region2to4.bmp') # RGB
        self.region2to4 = cv2.cvtColor(self.region2to4, cv2.COLOR_BGR2GRAY) # Grayscale
        self.region3to3 = cv2.imread('../MilestoneNov25/Maps/Region3to3.bmp') # RGB
        self.region3to3 = cv2.cvtColor(self.region3to3, cv2.COLOR_BGR2GRAY) # Grayscale
        self.region3to1 = cv2.imread('../MilestoneNov25/Maps/Region3to1.bmp') # RGB
        self.region3to1 = cv2.cvtColor(self.region3to1, cv2.COLOR_BGR2GRAY) # Grayscale
        self.region4to1 = cv2.imread('../MilestoneNov25/Maps/Region4to1.bmp') # RGB
        self.region4to1 = cv2.cvtColor(self.region4to1, cv2.COLOR_BGR2GRAY) # Grayscale
        self.region4to2 = cv2.imread('../MilestoneNov25/Maps/Region4to2.bmp') # RGB
    ...
    def update_log_file(self):
        if self.cur_region == 0:
            ...
            self.start_time = time.time()
            out_string = "Region:"+str(self.cur_region)+" | GPS:"+str(self.cur_gps)+" | Angle:"+str(self.cur_angle)+" | Duration:"+str(duration)
